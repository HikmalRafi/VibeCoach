...
/*void computeAngles()
{
	roll = asinf(-2.0f * (q1*q3 - q0*q2)) * R2D;
	pitch = atan2f(q0*q1 + q2*q3, 0.5f - q1*q1 - q2*q2) * R2D;
...
void computeAngles()
{
    float val = -2.0f * (q1*q3 - q0*q2);

    // clamp biar aman dari NaN
    val = fminf(fmaxf(val, -1.0f), 1.0f);
    roll = asinf(val) * R2D;

    pitch = atan2f(q0*q1 + q2*q3, 0.5f - q1*q1 - q2*q2) * R2D;

    // pakai val yang sama (SUDAH CLAMP)
    if (fabs(val) >= 1.0f)
        pitch = copysign(90.0f, val);
